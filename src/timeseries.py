...
def create_tser(wrfout,domain,date,names,lons,lats,nrun):
    ...
    for name,lon,lat in zip(names,lons,lats):
        ...
        t = d.time/3600.
        t = t-(nrun-1)*24 if nrun==2 else t # HACK
        ...
        pl.figtext(0.5,0.93,'%s'%(d.date[0]),size=8,ha='center')
        ...
        for i in range(t.size):
            if(d.wstar[i] > 0.0):
                pl.bar(t[i]-0.2,d.zi[i],width=0.4,bottom=zs,color=wup((np.floor(d.wstar[i])+0.5)/wm),edgecolor='none')    
                pl.bar(t[i]-0.225,d.ct[i]-d.zi[i],width=0.45,bottom=d.zi[i]+zs,color='k',alpha=0.3,edgecolor='none')    
        ...
        pl.ylabel('z [m AMSL]')
        pl.xticks(np.arange(0,24.0001,2))
        ...
        tmp    = str((nrun-1)*24).zfill(5) # HACK

        name = figRoot + date + '/' + tmp + '_d' + str(domain) + '_tser_' + name + '.png'
        pl.savefig(name)
